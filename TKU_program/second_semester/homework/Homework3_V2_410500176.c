...
stack_ptr input_string(FILE *fp)
{
    ...
    Data data;
    int num = 0;       
    int is_num = 0; //代表是否為數字
    while(EOF!=(ch=fgetc(fp)) && ch != '\n'){
        if(get_token(ch)==operand){
            is_num = 1;
            num = ch-'0';
            while(EOF!=(ch=fgetc(fp)) && ch != '\n' && get_token(ch)==operand){
                num*=10;
                num+=ch-'0';
            }
        } 
        if(get_token(ch)!=operand){
            if(is_num){ //字元輸入為符號時 判斷num是否存值，有的話push 然後再將字元push到stdin_
                data.num = num;
    ...
    Data zero = {operand, 0};   //假如第一個數字為負數 ex: -1-1 則變為 0-1-1
    if(ch!=EOF && input->data.operator==minus) push(&input, zero);
    /*
    負數未寫: 用input 去判斷括號內的負數 EX:(-1) -> -1 存入input->data.num 
    */
